...
template <typename MappingRecord>
void Chromap<MappingRecord>::GetRidRank(const std::string rid_order_path, const SequenceBatch &reference, std::vector<int> &rid_rank)
{
  ...
  std::ifstream file_stream(rid_order_path);
	...
	// we may have some rank without any rid associated with, this helps if cutstom list contains rid not in the reference`
	...
	if (k > ref_size) {
		chromap::Chromap<>::ExitWithMessage("Unknown chromsome names found in chromosome order file");
	}
...
template <typename MappingRecord> 
void Chromap<MappingRecord>::RerankCandidatesRid(std::vector<Candidate> &candidates) {
	int i;
	int size = candidates.size();
	for (i = 0; i < size; ++i) {
		uint64_t rid = (uint32_t)(candidates[i].position>>32);	
		rid = custom_rid_rank_[rid];
		candidates[i].position = (candidates[i].position & (uint64_t)0xffffffff) | (rid << 32);
	}
}
...
//#pragma omp taskloop num_tasks(num_threads_* num_threads_)
          for (uint32_t pair_index = 0; pair_index < num_loaded_pairs; ++pair_index) {
            ...
            if (!barcode_whitelist_file_path_.empty()) {
              current_barcode_is_whitelisted = CorrectBarcodeAt(pair_index, &barcode_batch, &thread_num_barcode_in_whitelist, &thread_num_corrected_barcode); 
            }
            if (current_barcode_is_whitelisted || output_mappings_not_in_whitelist_) {
              ...
              if (minimizers1.size() != 0 && minimizers2.size() != 0) {
                ...
                if ( !split_alignment_) {
                  supplementCandidateResult = SupplementCandidates(index, repetitive_seed_length1, repetitive_seed_length2, minimizers1, minimizers2, positive_hits1, positive_hits2, positive_candidates1, positive_candidates2, positive_candidates1_buffer, positive_candidates2_buffer, negative_hits1, negative_hits2, negative_candidates1, negative_candidates2, negative_candidates1_buffer, negative_candidates2_buffer);
		...
		if (output_mapping_in_SAM_) {
			...
			*ref_end_position = verification_window_start_position + read_start_site + mapping_end_position - 1;
			...
			if (!split_alignment_) {
				...
				BandedAlignPatternToText(reference.GetSequenceAt(rid) + verification_window_start_position, read + read_start_site, read_length, &mapping_end_position); 
				mapping_end_position += 1; // seems banded align's mapping end position is included?
			}
...
void Chromap<MappingRecord>::VerifyCandidatesOnOneDirection(Direction candidate_direction, const SequenceBatch &read_batch, uint32_t read_index, const SequenceBatch &reference, const std::vector<Candidate> &candidates, std::vector<std::pair<int, uint64_t> > *mappings, std::vector<int> *split_sites, int *min_num_errors, int *num_best_mappings, int *second_min_num_errors, int *num_second_best_mappings) {
  ...
  for (uint32_t ci = 0; ci < candidates.size(); ++ci) {
    ...
    //std::cerr << "ne3: " << num_errors << " " << mapping_end_position << " " << actual_num_errors << " "<< reference.GetSequenceNameAt(rid) <<" " << (int)candidates[ci].position << " " << position<<"\n";
		if (num_errors <= error_threshold_) {
			if (num_errors < *min_num_errors) {
				*second_min_num_errors = *min_num_errors;
				*num_second_best_mappings = *num_best_mappings;
			...
			if (candidate_direction == kPositive) {
				...
				if (split_alignment_ && !output_mapping_in_SAM_) {
					...
					mappings->emplace_back(num_errors, candidates[ci].position - gap_beginning); 
				} else {
...
template <typename MappingRecord>
void Chromap<MappingRecord>::VerifyCandidates(const SequenceBatch &read_batch, uint32_t read_index, const SequenceBatch &reference, const std::vector<std::pair<uint64_t, uint64_t> > &minimizers, const std::vector<Candidate> &positive_candidates, const std::vector<Candidate> &negative_candidates, std::vector<std::pair<int, uint64_t> > *positive_mappings, std::vector<int> *positive_split_sites, std::vector<std::pair<int, uint64_t> > *negative_mappings, std::vector<int> *negative_split_sites, int *min_num_errors, int *num_best_mappings, int *second_min_num_errors, int *num_second_best_mappings) {
...
int Chromap<MappingRecord>::BandedAlignPatternToTextWithDropOff(const char *pattern, const char *text, const int read_length, int *mapping_end_position, int *read_mapping_length) {
  ...
  *mapping_end_position = band_start_position;
  ...
  if (fail_beginning || (read_length > 60 && *mapping_end_position + 1 - error_threshold_ - min_num_errors < 30)) {
...
int Chromap<MappingRecord>::BandedAlignPatternToTextWithDropOffFrom3End(const char *pattern, const char *text, const int read_length, int *mapping_end_position, int *read_mapping_length) {
  ...
  for (; i < read_length; i++) {
    ...
    prev_num_errors_at_band_start_position = num_errors_at_band_start_position;
    num_errors_at_band_start_position += 1 - (D0 & lowest_bit_in_band_mask);
    /*printf("->%d %d %c %c", i, num_errors_at_band_start_position, pattern[read_length - 1 - i], text[read_length - 1 - i]) ;
    int tmp = num_errors_at_band_start_position;
    for (int j = 0; j < 2 * error_threshold_; j++) {
	    tmp = tmp + ((VP >> j) & (uint32_t) 1);
	    tmp = tmp - ((VN >> j) & (uint32_t) 1);
	    printf(" %d", tmp);
    }
    printf("\n");*/
    if (num_errors_at_band_start_position > 2 * error_threshold_) {
...
void Chromap<MappingRecord>::BandedTracebackToEnd(int min_num_errors, const char *pattern, const char *text, const int read_length, int *mapping_end_position) {
  ...
  int num_errors_at_band_start_position = 0;
  for (int i = 0; i < read_length; i++) {
    //printf("=>%d %d %c %c\n", i, num_errors_at_band_start_position, pattern[i + 2 * error_threshold_], text[i]) ;
    uint8_t pattern_base = SequenceBatch::CharToUint8(pattern[i + 2*error_threshold_]);
    Peq[pattern_base] = Peq[pattern_base] | highest_bit_in_band_mask;
    X = Peq[SequenceBatch::CharToUint8(text[i])] | VN;
    D0 = ((VP + (X & VP)) ^ VP) | X;
...
void ChromapDriver::ParseArgsAndRun(int argc, char *argv[]) {
  ...
  options.add_options("Output")
    ...
    ("output-mappings-not-in-whitelist", "Output mappings with barcode not in the whitelist")
  ...
  if (result.count("i")) {
    ...
    if (result.count("2") == 0) {
      if (output_mapping_in_PAF) {
        chromap::Chromap<chromap::PAFMapping> chromap_for_mapping(error_threshold, match_score, mismatch_penalty, gap_open_penalties, gap_extension_penalties, min_num_seeds_required_for_mapping, max_seed_frequencies, max_num_best_mappings, max_insert_size, mapq_threshold, num_threads, min_read_length, multi_mapping_allocation_distance, multi_mapping_allocation_seed, drop_repetitive_reads, trim_adapters, remove_pcr_duplicates, remove_pcr_duplicates_at_bulk_level, is_bulk_data, allocate_multi_mappings, only_output_unique_mappings, output_mappings_not_in_whitelist, Tn5_shift, split_alignment, output_mapping_in_BED, output_mapping_in_TagAlign, output_mapping_in_PAF, output_mapping_in_SAM, output_mapping_in_pairs, low_memory_mode, cell_by_bin, bin_size, depth_cutoff_to_call_peak, peak_min_length, peak_merge_max_length, reference_file_path, index_file_path, read_file1_paths, read_file2_paths, barcode_file_paths, barcode_whitelist_file_path, output_file_path, matrix_output_prefix, custom_rid_order_path, pairs_custom_rid_order_path);
