...
#pragma omp taskloop grainsize(grain_size)  // num_tasks(num_threads_* 50)
          ...
          //}
          // Update cache
          for (uint32_t pair_index = 0; pair_index < num_loaded_pairs / 2;
               ++pair_index) {
            if (num_reads_ >= 2 * 5000000 &&
                pair_index >= num_loaded_pairs / num_threads_) {
              break;
            }
            ...
            mm_to_candidates_cache.Update(
                ...
                mm_history2[pair_index].negative_candidates,
                mm_history2[pair_index].repetitive_seed_length);
            if (mm_history1[pair_index].positive_candidates.size() <
                mm_history1[pair_index].positive_candidates.capacity() / 2) {
              std::vector<Candidate>().swap(
                  mm_history1[pair_index].positive_candidates);
            }
            if (mm_history1[pair_index].negative_candidates.size() <
                mm_history1[pair_index].negative_candidates.capacity() / 2) {
              std::vector<Candidate>().swap(
                  mm_history1[pair_index].negative_candidates);
            }
            if (mm_history2[pair_index].positive_candidates.size() <
                mm_history2[pair_index].positive_candidates.capacity() / 2) {
              std::vector<Candidate>().swap(
                  mm_history2[pair_index].positive_candidates);
            }
            if (mm_history2[pair_index].negative_candidates.size() <
                mm_history2[pair_index].negative_candidates.capacity() / 2) {
              std::vector<Candidate>().swap(
                  mm_history2[pair_index].negative_candidates);
            }
          }
...
#pragma omp taskloop num_tasks(num_threads_ *num_threads_)
          ...
          for (uint32_t read_index = 0; read_index < num_loaded_reads / 2;
               ++read_index) {
