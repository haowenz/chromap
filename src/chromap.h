...
class Chromap {
 public:
  ...
  bool CorrectBarcodeAt(uint32_t barcode_index, SequenceBatch *barcode_batch, uint64_t *num_barcode_in_whitelist, uint64_t *num_corrected_barcode);
 ...
 private:
  ...
  std::string matrix_output_prefix_;
	...
	std::string pairs_custom_rid_order_path_; // the order for pairs format flipping
