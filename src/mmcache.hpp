...
class mm_cache_candidate_list {
 private:
  uint32_t *positions;
  uint8_t *counts;  // 0: the corresponding position is chr id. Otherwise, it is
                    // coordinate within the chr id
  uint32_t size;
 ...
 public:
  ...
  void Input(const std::vector<Candidate> &candidates) {
    ...
    for (i = 0; i < (int)actual_size; ++i) {
      if (i == 0 || (candidates[i].position >> 32) !=
                        (candidates[i - 1].position >> 32)) {
        counts[k] = 0;
  ...
  void Shift(int offset) {
    int i;
    for (i = 0; i < (int)size; ++i) {
      if (counts[i] != 0) positions[i] += offset;
    }
  }

  uint32_t GetActualSize() { return actual_size; }
};
 ...
 public:
  ...
  int Query(const std::vector<std::pair<uint64_t, uint64_t> > &minimizers,
    ...
    if (direction == 1) {
      ...
      cache[hidx].negative_candidate_list.Output(pos_candidates,
                                                 shift - read_len + 1);
      cache[hidx].positive_candidate_list.Output(neg_candidates,
                                                 -shift + read_len - 1);
      repetitive_seed_length = cache[hidx].repetitive_seed_length;
      return hidx;
    } else {
  ...
  uint64_t GetMemoryBytes() {
    ...
    for (i = 0; i < cache_size; ++i) {
      ret +=
          sizeof(cache[i]) + cache[i].minimizers.capacity() * sizeof(uint64_t);
  ...
  void PrintStats() {
    for (int i = 0; i < cache_size; ++i) {
      printf("%d %d %u ", cache[i].weight, cache[i].finger_print_cnt_sum,
             cache[i].positive_candidate_list.GetActualSize() +
