...
struct PAFMapping {
  ...
  bool operator<(const PAFMapping& m) const {
    return std::tie(fragment_start_position, fragment_length, mapq, read_id, read_length) < std::tie(m.fragment_start_position, m.fragment_length, m.mapq, m.read_id, m.read_length);
  }
  bool operator==(const PAFMapping& m) const {
...
class PAFOutputTools<PAFMapping> : public OutputTools<PAFMapping> {
  inline void AppendMapping(uint32_t rid, const SequenceBatch &reference, const PAFMapping &mapping) {
    ...
    std::string strand = (mapping.mapq & 1) == 1 ? "+" : "-";
    uint32_t mapping_end_position = mapping.fragment_start_position + mapping.fragment_length;
    this->AppendMappingOutput(mapping.read_name + "\t" + std::to_string(mapping.read_length) + "\t" + std::to_string(0) + "\t" + std::to_string(mapping.read_length) + "\t" + strand + "\t" + std::string(reference_sequence_name) + "\t" + std::to_string(reference_sequence_length) + "\t" + std::to_string(mapping.fragment_start_position) + "\t" + std::to_string(mapping_end_position) + "\t" + std::to_string(mapping.read_length) + "\t" + std::to_string(mapping.fragment_length) + "\t" + std::to_string(mapping.mapq >> 1) + "\n");
  }
};
...
class PAFOutputTools<MappingWithBarcode> : public OutputTools<MappingWithBarcode> {
  inline void AppendMapping(uint32_t rid, const SequenceBatch &reference, const MappingWithBarcode &mapping) {
    ...
    std::string strand = (mapping.mapq & 1) == 1 ? "+" : "-";
    uint32_t mapping_end_position = mapping.fragment_start_position + mapping.fragment_length;
    this->AppendMappingOutput(std::to_string(mapping.read_id) + "\t" + std::to_string(mapping.fragment_length) + "\t" + std::to_string(0) + "\t" + std::to_string(mapping.fragment_length) + "\t" + strand + "\t" + std::string(reference_sequence_name) + "\t" + std::to_string(reference_sequence_length) + "\t" + std::to_string(mapping.fragment_start_position) + "\t" + std::to_string(mapping_end_position) + "\t" + std::to_string(mapping.fragment_length) + "\t" + std::to_string(mapping.fragment_length) + "\t" + std::to_string(mapping.mapq >> 1) + "\n");
  }
};
